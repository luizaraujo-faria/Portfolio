...
const MenuBar = ({ scrolled, className }) => {
    ...
    return <aside className={`w-full h-screen transition-all duration-300 fixed z-40 left-0 top-0 bg-black flex items-center justify-center flex-col gap-16 p-2 md:w-2/4 lg:w-2/6 dark:bg-white ${className}`}>

        <header className={`w-[95%] h-14 flex items center justify-end p-5 bg-quaternary rounded-sm transition-all duration-300 ${scrolled ? 'mt-0' : 'mt-5'} dark:bg-quintenary`}>
        ...
        <nav className='w-[95%] h-3/4 flex items-center justify-start p-5 bg-quaternary rounded-sm dark:bg-quintenary'>
            <ul className='w-full text-xl text-white font-light flex flex-col gap-5 dark:text-black'>
                ...
                <li>
                    <Link to='/main#about' className='flex items-center justify-start gap-5 group w-full h-8 hover:bg-black dark:hover:bg-white'>
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="size-6 text-primary transition-all duration-300 dark:text-black group-hover:mb-5">
