...
const ContactSection = () => {
  ...
  return (
    <section id="contact" className="w-full h-screen bg-black relative z-20 flex items-center justify-start flex-col gap-16 lg:h-[115vh] dark:bg-white">
      <form ref={form} onSubmit={sendEmail} className="w-[90%] h-5/6 bg-quaternary rounded-lg mt-[-15%] flex items-center justify-center flex-col p-2 gap-5 shadow-[1px_1px_10px_] shadow-[#0f0f0f75] md:w-3/4 md:mt-[-10%] lg:w-2/4 lg:h-[90%] lg:gap-8 xl:h-[95%] xl:mt-[-8%] dark:bg-quintenary">
        <h3 className="text-4xl text-primary font-extralight lg:text-6xl dark:text-black">Contate-me</h3>

        <div className="w-full flex items-center justify-center flex-col gap-5">
          <input name='name' type="text" className="w-[90%] h-10 shadow-[0px_0px_5px_] shadow-[#0f0f0f75] bg-black rounded-sm p-5 text-base text-primary placeholder:text-primary placeholder:text-base -outline-offset-1 dark:border-black dark:text-black dark:placeholder:text-black dark:bg-white" placeholder="Nome" required />
          <input name='email' type="email" className="w-[90%] h-10 shadow-[0px_0px_5px_] shadow-[#0f0f0f75] bg-black rounded-sm p-5 text-base text-primary placeholder:text-primary placeholder:text-base -outline-offset-1 dark:border-black dark:text-black dark:placeholder:text-black dark:bg-white" placeholder="Email" required />
          <input name='number' type="tel" className="w-[90%] h-10 shadow-[0px_0px_5px_] shadow-[#0f0f0f75] bg-black rounded-sm p-5 text-base text-primary placeholder:text-primary placeholder:text-base -outline-offset-1 dark:border-black dark:text-black dark:placeholder:text-black dark:bg-white" placeholder="Telefone" required />
          <textarea name='message' className="w-[90%] h-72 shadow-[0px_0px_5px_] shadow-[#0f0f0f75] bg-black rounded-sm resize-none p-5 text-base text-primary placeholder:text-primary placeholder:text-base -outline-offset-1 dark:border-black dark:text-black dark:placeholder:text-black dark:bg-white" placeholder="Mensagem" required></textarea>
        </div>
